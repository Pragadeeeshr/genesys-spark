...
@Component({
  styleUrl: 'gux-tab-advanced-list-legacy.scss',
  tag: 'gux-tab-advanced-list-legacy',
  shadow: false
})
export class GuxTabAdvancedListLegacy {
  ...
  @State()
  tabTriggers: NodeListOf<HTMLGuxTabAdvancedLegacyElement>;
    ...
    switch (event.key) {
      ...
      case 'ArrowDown':
        ...
        if (
          ...
          const allNodes = parentNode.querySelectorAll(
  ...
  private setTabTriggers(): void {
    this.tabTriggers = this.root.querySelectorAll('gux-tab-advanced-legacy');
    if (this.tabTriggers) {
  ...
  createSortable() {
    this.sortableInstance = new Sortable(
      ...
      {
        animation: 250,
        draggable: 'gux-tab-advanced-legacy',
        filter: '.ignore-sort',
  ...
  emitSortChanged() {
    const tabIds = Array.from(
      this.root.querySelectorAll('gux-tab-advanced-legacy')
  ...
  scrollLeft() {
    writeTask(() => {
      this.root
        .querySelector('.gux-scrollable-section')
        .scrollBy(
          -this.root.querySelector('gux-tab-advanced-legacy')?.scrollWidth,
          0
        );
    });
  }
  ...
  componentWillRender() {
    const tabs: HTMLGuxTabAdvancedLegacyElement[] = Array.from(
      this.root.querySelectorAll('gux-tab-advanced-legacy')
    );
